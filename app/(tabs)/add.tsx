      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : "height"} keyboardVerticalOffset={Platform.OS === "ios" ? 88 : 0}>
        ...
        >
          <Section title="Property" open={open.property} onToggle={() => toggleSection("property")}>
            ...
            <Field label="Street Address" fieldKey="streetAddress" inputRefs={inputRefs} onNext={focusNext} value={draft.streetAddress} onChangeText={(t) => setDraft((d) => ({ ...d, streetAddress: t }))} />
            <View style={{ gap: 3, marginBottom: 8 }}>
              <Text style={{ color: colors.textSecondary, fontSize: 12, fontWeight: "600" }}>Zip Code{zipLooking ? " (looking up…)" : ""}</Text>
              <TextInput
                ref={(r) => { if (r) inputRefs.current["zipCode"] = r; }}
                value={draft.zipCode} onChangeText={(t) => setDraft((d) => ({ ...d, zipCode: t }))}
                keyboardType="number-pad" maxLength={5}
                placeholder="" placeholderTextColor={colors.textSecondary}
                style={{ backgroundColor: colors.cardHover, borderWidth: 1, borderColor: colors.border, borderRadius: 8, paddingHorizontal: 10, paddingVertical: 8, color: colors.textPrimary, fontSize: 14 }}
              />
            </View>
            <Field label="City" fieldKey="city" inputRefs={inputRefs} onNext={focusNext} value={draft.city} onChangeText={(t) => setDraft((d) => ({ ...d, city: t }))} />
            <Field label="State" fieldKey="state" inputRefs={inputRefs} onNext={focusNext} value={draft.state} onChangeText={(t) => setDraft((d) => ({ ...d, state: t }))} />
            ...
            {isAptCondoCoop && <Field label="Floor Number" fieldKey="floorNumber" inputRefs={inputRefs} onNext={focusNext} value={draft.floorNumber} onChangeText={(t) => setDraft((d) => ({ ...d, floorNumber: t }))} keyboardType="number-pad" />}
          ...
          {toggles.children && (
            <Section title="Schools" open={open.schools} onToggle={() => toggleSection("schools")}>
              ...
              <Field label="Rating (0–10)" fieldKey="elementaryRating" inputRefs={inputRefs} onNext={focusNext} value={draft.elementaryRating} onChangeText={(t) => setDraft((d) => ({ ...d, elementaryRating: t }))} keyboardType="decimal-pad" />
              ...
              <Text style={{ color: colors.textSecondary, fontSize: 11, fontWeight: "700", letterSpacing: 0.5, marginTop: 8, marginBottom: 2 }}>MIDDLE SCHOOL</Text>
              ...
              <Field label="Rating (0–10)" fieldKey="middleRating" inputRefs={inputRefs} onNext={focusNext} value={draft.middleRating} onChangeText={(t) => setDraft((d) => ({ ...d, middleRating: t }))} keyboardType="decimal-pad" />
              ...
              <Text style={{ color: colors.textSecondary, fontSize: 11, fontWeight: "700", letterSpacing: 0.5, marginTop: 8, marginBottom: 2 }}>HIGH SCHOOL</Text>
              ...
              <Field label="Rating (0–10)" fieldKey="highRating" inputRefs={inputRefs} onNext={focusNext} value={draft.highRating} onChangeText={(t) => setDraft((d) => ({ ...d, highRating: t }))} keyboardType="decimal-pad" />
          ...
          <Pressable onPress={handleSave} disabled={saving} style={{ backgroundColor: colors.primaryBlue, borderRadius: 12, paddingVertical: 14, alignItems: "center", marginTop: 8 }}>
            {saving ? <ActivityIndicator color="#fff" /> : <Text style={{ color: "#fff", fontSize: 16, fontWeight: "700" }}>Save Listing</Text>}
          </Pressable>
        </ScrollView>
      </KeyboardAvoidingView>
